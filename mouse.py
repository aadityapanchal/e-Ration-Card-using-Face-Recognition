...
#Python object hierarchy is converted into a byte stream,
import pickle
from threading import Thread
#if the mouse cursor is in the upper left corner of the screen. If you lose control and need to stop the current PyAutoGUI function, keep moving the mouse cursor up and to the left. To disable this feature, set FAILSAFE to False
gui.FAILSAFE = False
...
def start_mouse():
    ...
    while True:
        _, img = cam.read()
        img = cv2.flip(img, 1)
        imgHSV = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)  #most suitable for color based image segmentation
